...
/**
   ...
   * when we have referenced keys, as there can be different ON DELETE/ON UPDATE
     actions
*/
TABLE::replace_execution_result TABLE::replace_row(uint key_nr, COPY_INFO *info,
                                                   bool use_triggers,
                                                   bool versioned)
{
  replace_execution_result result{};
  if (last_uniq_key(this, key_nr) &&
      !file->referenced_by_foreign_key() && use_triggers)
  {
    DBUG_PRINT("info", ("Updating row using ha_update_row()"));
    if (this->versioned(VERS_TRX_ID))
    {
      bitmap_set_bit(write_set, vers_start_field()->field_index);
      file->column_bitmaps_signal();
      vers_start_field()->store(0, false);
    }
    result.error= file->ha_update_row(record[1], record[0]);
    ...
    if (likely(!result.error))
    {
      ...
      if (!file->has_transactions())
        in_use->transaction->stmt.modified_non_trans_table= TRUE;
      if (versioned)
      {
        store_record(this, record[2]);
        result.error= vers_insert_history_row(this);
  ...
  else
  {
    ...
    if (triggers && triggers->process_triggers(in_use, TRG_EVENT_DELETE,
    ...
    if (!versioned)
      result.error= file->ha_delete_row(record[1]);
    else
    {
      store_record(this, record[2]);
    ...
    if (!versioned)
      ...
      info->updated++;
    if (!file->has_transactions_and_rollback())
      in_use->transaction->stmt.modified_non_trans_table= TRUE;
    if (triggers && triggers->process_triggers(in_use, TRG_EVENT_DELETE,
...
int write_record(THD *thd, TABLE *table, COPY_INFO *info, select_result *sink)
{
      ...
      info->handle_duplicates == DUP_UPDATE)
  {
	...
	DBUG_ASSERT(table->insert_values != NULL);
      ...
      {
        auto result= table->replace_row(key_nr, info, table->triggers &&
